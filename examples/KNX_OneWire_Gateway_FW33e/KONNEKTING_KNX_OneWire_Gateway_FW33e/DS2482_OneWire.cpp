...
DS2482_OneWire::DS2482_OneWire(uint8_t address)
{
	// Address is determined by two pins on the DS2482 AD1/AD0
	// Pass 0b00, 0b01, 0b10 or 0b11
...
bool DS2482_OneWire::selectChannel(uint8_t channel)
{
	uint8_t ch, ch_read;
...
uint8_t DS2482_OneWire::getAddress()
{
	return mAddress;
}

uint8_t DS2482_OneWire::getError()
{
	return mError;
}

// Helper functions to make dealing with I2C side easier
void DS2482_OneWire::begin()
{
	Wire.beginTransmission(mAddress);
}

uint8_t DS2482_OneWire::end()
{
...
uint8_t DS2482_OneWire::checkPresence()
{
	begin();
...
void DS2482_OneWire::deviceReset()
{
	begin();
...
void DS2482_OneWire::setReadPointer(uint8_t readPointer)
{
	begin();
...
uint8_t DS2482_OneWire::readStatus()
{
...
uint8_t DS2482_OneWire::readData()
{
...
uint8_t DS2482_OneWire::readConfig()
{
	setReadPointer(DS2482_POINTER_CONFIG);
	return readByte();
}

void DS2482_OneWire::setStrongPullup()
{
	writeConfig(readConfig() | DS2482_CONFIG_SPU);
}

void DS2482_OneWire::clearStrongPullup()
{
	writeConfig(readConfig() & !DS2482_CONFIG_SPU);
}

void DS2482_OneWire::setActivePullup()
{
  writeConfig(readConfig() | DS2482_CONFIG_APU);
}

void DS2482_OneWire::clearActivePullup()
{
  writeConfig(readConfig() | !DS2482_CONFIG_APU);
}

// Churn until the busy bit in the status register is clear
uint8_t DS2482_OneWire::waitOnBusy()
{
	uint8_t status;
...
void DS2482_OneWire::writeConfig(uint8_t config)
{
	waitOnBusy();
	begin();
...
void DS2482_OneWire::wireWriteByte(uint8_t data, uint8_t power)
{
	waitOnBusy();
	if (power)
		setStrongPullup();
...
uint8_t DS2482_OneWire::wireReadByte()
{
	waitOnBusy();
	begin();
...
// As wireWriteBit
uint8_t DS2482_OneWire::wireReadBit()
{
	wireWriteBit(1);
	uint8_t status = waitOnBusy();
...
// 1-Wire skip
void DS2482_OneWire::wireSkip()
{
	wireWriteByte(WIRE_COMMAND_SKIP);
}

void DS2482_OneWire::wireSelect(const uint8_t rom[8])
{
...
void DS2482_OneWire::reset_search()
{
	wireResetSearch();
}

uint8_t DS2482_OneWire::search(uint8_t *newAddr)
{
...
uint8_t DS2482_OneWire::reset(void)
{
	return wireReset();
}

// Issue a 1-Wire rom select command, you do the reset first.
void DS2482_OneWire::select(const uint8_t rom[8])
{
	wireSelect(rom);
}

// Issue a 1-Wire rom skip command, to address all on bus.
void DS2482_OneWire::skip(void)
{
	wireSkip();
}
...
// Read a byte.
uint8_t DS2482_OneWire::read(void)
{
	return wireReadByte();
}

// Read a bit.
uint8_t DS2482_OneWire::read_bit(void)
{
	return wireReadBit();
}

// Write a bit.
void DS2482_OneWire::write_bit(uint8_t v)
{
	wireWriteBit(v);
}
